...
3. Каждый засчитанный поворот отмечается коротким звуковым сигналом — «дзынь». Количество сигналов
   показывает, насколько хорошо выполнен поворот:

   - **Один сигнал** — поворот дотянул минимум до 70% порога следующего уровня по всем метрикам
     навыка.

     [▶ Один сигнал](assets/user-guide/chime-1.wav)

   - **Два сигнала** — поворот полностью, по всем метрикам, достиг или превысил порог следующего
     уровня.

     [▶ Два сигнала](assets/user-guide/chime-2.wav)
...
4. Уровень засчитывается, когда вы проедете **подряд** нужное число поворотов (обычно 8 — указано на
   экране цели), каждый из которых получил **два сигнала** (100% по всем метрикам навыка).
...
обычным GF Score — средним по недавним спускам этой же техникой. Один сигнал — поворот дотянул минимум
до 70% от обычного счёта, два — сравнялся с ним или превысил.
