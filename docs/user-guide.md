...
### 2.4 Калибровочный спуск - настройка распознавания поворотов

Это отдельная калибровка, не связанная с «нулём» датчика из раздела 2.3, и делать для неё ничего не
нужно. Чтобы отличать настоящий поворот от покачивания на почти прямом участке, приложению нужен
порог - насколько угол закантовки должен откатиться от пика, чтобы поворот засчитался. У разных
лыжников и разной техники он разный, поэтому подбирается автоматически по вашей записи.

- **Первый спуск в каждой технике** («Карвинг», «Короткий радиус», «Параллельные лыжи») -
  калибровочный. Приложение записывает его целиком, в конце спуска подбирает порог и задним числом
  считает по нему метрики и GF Score этого же спуска - он не пропадает.
- **Порог сохраняется на телефоне** отдельно для каждой техники и дальше используется сразу, с
  первого поворота.
- **Порог подстраивается после каждого спуска.** В конце спуска приложение подбирает порог заново и
  сдвигает сохранённый на 30% в его сторону. Один неудачный спуск почти не влияет, а если поменялась
  техника, ботинки или крепления, порог подстроится примерно за 5 спусков. Сам спуск считается по
  порогу, с которым он начался.
- **Сбросить калибровку** можно в профиле лыжника (раздел [3](#3-профиль-лыжника)) - кнопка
  **«Сбросить»** в карточке «Распознавание поворотов». После сброса следующий спуск в каждой
  технике снова будет калибровочным. Это полезно, если на телефоне катается другой лыжник.
- **Если порог подобрать не удалось** (например, на спуске было мало поворотов или они были совсем
  пологие) - у этого спуска не будет метрик и GF Score, а калибровка повторится на следующем спуске
  в той же технике. Для надёжной калибровки проедьте первый спуск обычными, выраженными поворотами -
  не траверсом и не «змейкой» на прямых лыжах.
...
| **Распознавание поворотов** | кнопка «Сбросить» - порог подбирается заново (раздел [2.4](#24-калибровочный-спуск---настройка-распознавания-поворотов)) |
...
![Профиль лыжника - вес и распознавание поворотов](assets/user-guide/10-skier-profile-scroll.jpg)
...
**«Тренировать»**, запускающая тренировку на склоне. Во время тренировки она превращается в **«Стоп»** - нажмите повторно, чтобы завершить сессию. Если перейти на другой навык той же дисциплины,
...
| GF Score не считается, метрики пустые | Датчики подключены? Без них считаются только GPS-метрики (раздел «Запись трека без датчиков»). На спуске должно быть не меньше 8 поворотов. Если это первый спуск в этой технике - не удалась калибровка распознавания поворотов, она повторится на следующем спуске (раздел [2.4](#24-калибровочный-спуск---настройка-распознавания-поворотов)) |
